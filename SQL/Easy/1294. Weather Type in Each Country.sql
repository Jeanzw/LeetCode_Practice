...
def weather_type(countries: pd.DataFrame, weather: pd.DataFrame) -> pd.DataFrame:
    merge = pd.merge(countries,weather,on = 'country_id')
    merge = merge[(merge['day'] >= '2019-11-01') & (merge['day'] <= '2019-11-30')]
    merge = merge.groupby(['country_name'],as_index = False).weather_state.mean()
    merge['weather_type'] = np.where(merge['weather_state'] <= 15, 'Cold',

...
def friday_purchases(purchases: pd.DataFrame) -> pd.DataFrame:
    ...
    merge = pd.merge(df,purchases, on = 'purchase_date', how = 'left')  
    merge = merge.groupby(['week_of_month','purchase_date'],as_index = False).amount_spend.sum().fillna(0)
    return merge.rename(columns = {'amount_spend':'total_amount'}).sort_values('week_of_month')